...
const Arts = () => {
  ...
  const imagesArts = images.filter((img) => img.category === 'Arts');
  return (
    <section className="max-w-7xl mx-auto px-4 py-8 imgs-section">
      <div className="flex justify-center gap-4 mb-8">
        ...
        <button className='bg-[#000000] cursor-pointer text-white px-4 py-2 rounded-md' onClick={() => router.push('/photography')}>Photography</button>
      </div>
      <h1 className='font-[family-name:var(--font-moonwalk)] text-xl text-center'>Arts</h1>
      <div style={styles.gridContainer}>
        {imagesArts.map((img, index) => (
